...
\paragraph{Practical workflow.} Draw every $z$-axis first, then each common normal $x$-axis (pointing from $z_{k-1}$ toward $z_k$, so $a_k\ge0$; a reversed $x_k$ forces a nonstandard negative $a_k$), and finally obtain $y$ by the right-hand rule. A dimension measured along a joint axis is a $d$: for a roll joint about its own link, that link's length goes in $d$ and the preceding $a$ is $0$. Check every $x_k\perp z_{k-1}$, and walk the tip at a drawn pose to confirm the figure's dimension sums. Tabulate $(\theta,d,a,\alpha)$ before multiplying matrices. A D--H frame assignment is not unique (pp.~61--65), but a consistent assignment yields the same physical tool pose.
...
Common analytic steps: use geometric offsets to isolate a wrist, square and add
coupled $x,y$ equations to eliminate an angle, then recover angles with
$\operatorname{atan2}(\sin\theta,\cos\theta)$. For a planar 2R arm,
$\cos\theta_2=(x^2+y^2-\ell_1^2-\ell_2^2)/(2\ell_1\ell_2)$:
its value must lie in $[-1,1]$, and the two signs of $\sin\theta_2$
usually give two elbow branches. Dividing to form $\tan\theta$ loses quadrant
information and fails at a zero denominator. Check every branch by forward substitution.
...
\textbf{Learner's upward-positive D--H convention.} For D--H rows
$(\pi/2,d_1,0,\pi/2)$, $(-\pi/2,d_2,0,\pi/2)$,
$(\pi+\theta_3,0,0,\pi/2)$, $(\theta_4-\pi/2,L,0,0)$,
the approach is $(c_3,0,s_3)^\trans$, so
\[
 \mathbf w=(d_2+Lc_3,\ 0,\ d_1+Ls_3,\ e^{\theta_4/\pi}c_3,\ 0,\ e^{\theta_4/\pi}s_3)^\trans.
\]
The six entries are three tip-position and three scaled approach-vector
coordinates, not six independent joints. Planar motion enforces
$\widehat w_2=\widehat w_5=0$ for any reachable target.
With $\rho=\sqrt{\widehat w_4^2+\widehat w_6^2}>0$,
\[
 \theta_3=\operatorname{atan2}(\widehat w_6,\widehat w_4),\quad
 \theta_4=\pi\ln\rho,\quad
 d_2=\widehat w_1-L\widehat w_4/\rho,\quad
 d_1=\widehat w_3-L\widehat w_6/\rho.
\]
Require $\widehat w_2=\widehat w_5=0$ and respect joint limits. Check the
full D--H rotation with $R^\trans R=I$ and $\det R=+1$ before using its columns.
\par\emph{Source: \texttt{resources/exercises-kinematics-not-for-submission.pdf}, Problem~1, p.~1; learner-supplied frame assignment, 2026-09-23.}
...
For Problem~1(d) with the learner's upward-positive convention, choose
$q=(d_1,d_2,\theta_3,\theta_4)^\trans$ and $E=e^{\theta_4/\pi}$.
Rows index the six outputs $w_i$; columns index the four joints $q_j$:
\[
 V=\left[\frac{\partial w_i}{\partial q_j}\right]_{6\times4}
 =\begin{bmatrix}
 0&1&-Ls_3&0\\
 0&0&0&0\\
 1&0&Lc_3&0\\
 ...
 \end{bmatrix}.
\]
This differentiates the course's $w$, not the alternative spatial twist
$(\dot p,\boldsymbol\omega)$.
\par\emph{Source: \texttt{resources/exercises-kinematics-not-for-submission.pdf}, Problem~1(d), p.~1; \texttt{resources/lecture-01-manipulator-kinematics.pdf}, pp.~133--141.}
