...
\end{itemize}
\textbf{An unbounded feasible region does not mean an unbounded objective.} These are two separate questions. A ``$\geq$''-constrained minimisation, for instance
\[
 \min\ 2x_1+3x_2\quad\text{s.t.}\quad x_1+x_2\geq4,\ \ x_1+3x_2\geq6,\ \ x_1,x_2\geq0,
\]
has a feasible region that extends to infinity up and to the right, yet the objective is bounded below and the minimum is finite. The region's corners are $(0,4)$, $(3,1)$, $(6,0)$ with objective values $12$, $9$, $12$, so the optimum is $(3,1)$, $z=9$ --- the intersection of the two requirement lines (subtract them: $2x_2=2$). The objective is unbounded only if you can move to infinity \emph{along a feasible direction while the objective keeps improving}; here every feasible direction that decreases $2x_1+3x_2$ eventually leaves the region. Test the improving direction, not the size of the region. For a minimisation with all ``$\geq$'' resource constraints the region is typically unbounded and the answer is still one of its corners.
...
\]
has all three constraint lines through $(0,4)$: $5(4)=20$, $6(4)=24$, $4(4)=16$. Entering $x_2$, the ratio test gives $20/5=24/6=16/4=4$ --- a three-way tie. Whichever row is pivoted, the optimal tableau has $x_2=4$, objective $28$, and the other two slacks stuck at zero. Note that $(0,4)$ is still optimal even though moving along the line $2x_1+6x_2=24$ would raise the objective ($5x_1+7x_2=28+\tfrac83x_1$): that direction immediately violates $4x_1+5x_2\leq20$, so it is not a feasible edge. At a degenerate corner an improving \emph{direction} can exist while no improving \emph{feasible edge} does --- which is exactly why the tie is worth spotting. Recognising a degenerate tableau (a ratio-test tie, or a zero in the basic-variable column at optimality) is a common short-answer target.
...
\paragraph{Reading special cases off a tableau.} Once you can identify which columns are basic (their column is a unit vector; the variable equals that row's RHS) and which are nonbasic (set to zero), three diagnoses come straight from the tableau. Keep them apart --- they are easy to confuse:
\begin{itemize}
\item \textbf{Degeneracy:} a \emph{basic} variable equals zero (a zero in the RHS column of a basic row). The corner is over-determined; the next pivot may not move. Spotted also as a tie in the ratio test.
\item \textbf{Alternative optima:} the tableau is already optimal (no negative reduced cost, using the $z-c^\T x$ convention) \emph{and} a \emph{nonbasic} variable has reduced cost exactly zero. Pivoting it in changes the vertex but not $z$, so a whole optimal edge exists. The zero is on a \emph{nonbasic} column --- a zero under a \emph{basic} column is normal and means nothing.
\item \textbf{Unboundedness:} a nonbasic variable has a negative reduced cost (it should enter) but every entry in its column is $\leq0$, so the ratio test has no eligible row. The objective rises without limit.
\end{itemize}
The trap is swapping the first two: degeneracy is a \emph{basic} variable at zero (a value); alternative optima is a \emph{nonbasic} variable with zero \emph{reduced cost} (a $z$-row entry). Different columns, different rows, different meaning.
...
\appendix
\section{Original worked solutions: the Week~3 exercise class}
\label{app:exercises}
The five problems below were worked by hand by the lecturer in the Week~3 exercise lecture; the scans are \texttt{resources/EE6204\_Exercises-part1.pdf}. They are reproduced here only as a visual reference --- each linear program is typeset and re-derived in the graphical-method, simplex, and integer-programming sections above. Two caveats carried into those sections: Q1's gradient is annotated $\nabla f=[7;5]$ but should be $[5;7]$ (the optimum is unaffected), and the Q3 branch-and-bound tree stops at an unlabelled node, so its lower branches and the Q4--Q5 answers are completed in the notes rather than taken from the scan.

\begin{figure}[p]
  \centering
  \fbox{\includegraphics[page=1,width=0.78\linewidth]{EE6204_Exercises-part1.pdf}}
  \caption{Q1 --- graphical solution of $\max\ 5x_1+7x_2$ subject to $4x_1+5x_2\leq20$, $2x_1+6x_2\leq24$, $6x_1+4x_2\leq16$. Optimum $(x_1,x_2)=(0,4)$, $f=28$; all three constraint lines pass through the optimum (a degenerate vertex).}
\end{figure}

\begin{figure}[p]
  \centering
  \fbox{\includegraphics[page=2,width=0.78\linewidth]{EE6204_Exercises-part1.pdf}}
  \caption{Q2 --- the same LP by the simplex method. Entering $x_2$ gives a three-way ratio-test tie ($20/5=24/6=16/4$); the optimal tableau is degenerate, with two slacks at zero. Same optimum, $z=28$.}
\end{figure}

\begin{figure}[p]
  \centering
  \fbox{\includegraphics[page=3,width=0.78\linewidth]{EE6204_Exercises-part1.pdf}}
  \caption{Q3 --- branch and bound for $\max\ 7x_1+5x_2$ subject to $4x_1+6x_2\leq18$, $2x_1+6x_2\leq9$, $6x_1+3x_2\leq27$, $x_1,x_2$ integer. Relaxation optimum $(4.5,0)$; the integer optimum is $(4,0)$, $z=28$ (tree completed in the integer-programming section).}
\end{figure}

\begin{figure}[p]
  \centering
  \fbox{\includegraphics[page=4,width=0.78\linewidth]{EE6204_Exercises-part1.pdf}}
  \caption{Q4 --- statement of $\max\ 200x+550y$ subject to $100x+250y\leq5000$, $x+y\leq50$ (graphical). Optimum $(0,20)$, value $11000$, derived in the worked-graphical-solution section.}
\end{figure}

\begin{figure}[p]
  \centering
  \fbox{\includegraphics[page=5,width=0.78\linewidth]{EE6204_Exercises-part1.pdf}}
  \caption{Q5 --- statement of the same LP as Q4, to be solved by the simplex method.}
\end{figure}
